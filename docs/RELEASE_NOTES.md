...
- Added a per-semester tag editor to the mobile app (`app/semester/tags.tsx`, a modal screen opened from a new "Tags" button in the semester header): Readings/Tasks tabs, each grouped into Pending and Done sections like the desktop's Tags tab. Tags can be added (inline name + swatch form per section), renamed (tap the name, commits on blur), recolored (tap the color dot to open the same fixed 8-color palette as the course form), deleted (behind a confirmation alert; items wearing the tag become ghosts via core, so progress stays stable), and reordered (↑/↓ controls, swap-based within the tag's section). Protected tags (pending/studied) show a lock note and can only be recolored — no rename, delete, or reorder — mirroring the desktop. All mutations go through the shared `@lectio/core` tag functions (`addTag`/`editTag`/`deleteTag`/`reorderTags`, now declared in the mobile ambient types), so the semester JSON stays byte-compatible with the desktop. No new dependencies.
