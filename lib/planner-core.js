...
(function (global, factory) {
  ...
  // the "done" section. Ghost items (status '__deleted__') count via the
  // section they were assigned before their tag was deleted.
  function courseProgress(course, semester) {
    ...
    const counts = (item, doneIds) =>
      item.status === '__deleted__' ? item._ghostSection === 'done' : doneIds.has(item.status);

    const doneR = readings.filter((r) => counts(r, rDoneIds)).length;
    const doneT = tasks.filter((t) => counts(t, tDoneIds)).length;
