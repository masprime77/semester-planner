...
- Added course sort controls to the mobile courses screen: a new ↑↓ header button (next to Edit) opens a bottom sheet (`src/components/SortMenu.tsx`, plain RN `<Modal>`, no new dependencies) offering the same six orders as the desktop's header dropdown with the same wording — Progress ↓/↑, A → Z, Z → A, Week ↑/↓ — with the active order highlighted. The list is fed through core's `sortedCourses` (week orders fall back to A → Z for course ordering, exactly as on desktop, since mobile has no weekly view yet), and progress sorts honor Study Mode via the same `courseProgress` flag. Sorting is display-only — `sortedCourses` returns a new array, so the semester JSON's course order is never modified. The choice persists across relaunches via the existing `prefs.getSortOrder`/`setSortOrder`, validated against core's `SORT_ORDERS` on restore (unknown values fall back to A → Z).
